...
<template>
  <div class="flex flex-wrap gap-2 p-1.5 bg-white/60 dark:bg-gray-900/40 rounded-2xl border border-gray-200/50 dark:border-gray-800/50 backdrop-blur-sm w-fit">
    <button
      v-for="tab in tabs"
      :key="tab.key"
      @click="emit('update:modelValue', tab.key)"
      class="relative inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300"
